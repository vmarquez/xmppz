...
class EventDecoder[T](elemCreator: ElemCreator[T]) extends FrameDecoder {
  ...
  override def decode(ctx: ChannelHandlerContext, channel: Channel, buf: ChannelBuffer): \/[ConnectionError, Seq[T]] = {
    var packets = List[T]()
    var incomingData = ""
    try {
      ...
      //println("incomingData = size = " + arr.size + "|{" + incomingData + "}")
      ...
      case ex: Exception =>
        ...
        ex.printStackTrace
        return ConnectionError(exception = Some(ex), message = "Data = " + incomingData + "\\n" + fmt).left
    }
    ...
    packets.right //butwhat if we get nothing? should we call .left?
  }
